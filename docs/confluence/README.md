...
To synchronize local Markdown specifications to live Atlassian Confluence Cloud Space `[HT]`, execute the automated publisher script:

```bash
python scripts/sync_to_confluence.py
```
* Authentication: Automatic via Azure Key Vault (`confluence-api-token`) or `CONFLUENCE_API_TOKEN` environment variable.
