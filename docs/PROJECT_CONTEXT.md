...
* **Architecture:** Cloud-Native Kubernetes (AKS Free Tier `aks-ht-bankc-p-cin-01` on `Standard_B4ms` 4 vCPUs, LiteLLM Multi-Model Proxy Gateway with Azure OpenAI `gpt-5.4-nano` + Google Gemini 2.0 Flash, Qdrant Vector DB on 4GB CSI Managed Disk, DPDP PII Auto-Masking, Azure Static Web Apps).
