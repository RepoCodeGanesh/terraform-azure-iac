...
### 38. Single-Node 30GB Ephemeral Disk Pressure Eviction & Automated containerd Pruning
* **Symptom:** Pods get evicted with `Evicted: The node was low on resource: ephemeral-storage` and new pods fail to schedule with `0/1 nodes are available: 1 node(s) had untolerated taint(s) [node.kubernetes.io/disk-pressure:NoSchedule]`.
* **Root Cause:** In resource-constrained nodes (`aks_os_disk_size_gb = 30`), Linux OS (~10GB) and Azure system images (~8GB) leave only ~12GB. Successive CI/CD deployments leave dangling/unused image layers in containerd cache, pushing disk consumption past the 85% Kubelet threshold and triggering emergency pod evictions.
* **Resolution:** (1) Enhanced `finops-single-node-tuner.yaml` with a continuous failed/evicted pod purger across all namespaces. (2) Added `finops-disk-cleaner` CronJob in `kube-system` running every 2 hours with host PID/root mount executing `chroot /host /usr/bin/crictl rmi --prune`. Drops disk usage from >85% down to ~34% (freeing 19+ GB) with $0 extra storage cost.
